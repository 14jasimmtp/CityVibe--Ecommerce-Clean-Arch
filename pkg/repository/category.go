...
func NewCategoryRepo(db *gorm.DB) interfaceRepo.CategoryRepo{
	return &CategoryRepo{DB: db}
}

func (clean *CategoryRepo) GetCategory() ([]domain.Category, error) {
...
func (clean *CategoryRepo) AddCategory(category models.Category) (domain.Category, error) {
	var categore string
...
func (clean *CategoryRepo) DeleteCategory(id string) error {
	...
	if query := db.DB.Raw(`UPDATE products SET deleted = true WHERE category_id = ?`, category_id).Error; err != nil {
		return query
	}
	return nil
}
func (clean *CategoryRepo) UpdateCategory(current string, new string) (domain.Category, error) {
	if db.DB == nil {
...
func (clean *CategoryRepo) CheckCategory(current string) (bool, error) {
	var count int
