...
type PaymentHandler struct {
	PaymentUsecase interfaceUsecase.PaymentUsecase
}

func NewPaymentHandler(usecase interfaceUsecase.PaymentUsecase) *PaymentHandler{
...
func (clean *PaymentHandler) ExecuteRazorPayPayment(c *gin.Context) {
	orderID, err := strconv.Atoi(c.Query("order_id"))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "error from orderID"})
		return
	}
	paymentMethodID, err := clean.PaymentUsecase.PaymentMethodID(orderID)
	if err != nil {
	...
	if paymentMethodID == 2 {
		payment, _ := clean.PaymentUsecase.PaymentAlreadyPaid(orderID)
		if payment {
			c.HTML(http.StatusOK, "pay.html", nil)
			return

		}
		orderDetail, Razor_id, err := clean.PaymentUsecase.MakePaymentRazorPay(orderID)
		if err != nil {
			c.HTML(http.StatusNotFound, "notfound.html", nil)
			return
		}
...
func (clean *PaymentHandler) VerifyPayment(c *gin.Context) {
	...
	Order, err := clean.PaymentUsecase.VerifyPayment(Verify, order_id)
	if err != nil {
