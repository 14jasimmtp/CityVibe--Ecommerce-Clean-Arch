...
func NewProductHandler(usecase interfaceUsecase.ProductUsecase) *ProductHandler{
...
func (clean *ProductHandler) AddProduct(c *gin.Context) {
	...
	NewProduct, err := clean.ProductUsecase.AddProduct(product, image)
	if err != nil {
...
func (clean *ProductHandler) EditProductDetails(c *gin.Context) {
	var product models.AddProduct
	...
	UpdatedProduct, err := clean.ProductUsecase.EditProductDetails(id, product)
	if err != nil {
	...
	// err:=clean.ProductUsecase.EditProductDetails(product)
}
...
func (clean *ProductHandler) DeleteProduct(c *gin.Context) {
	id := c.Param("id")

	err := clean.ProductUsecase.DeleteProduct(id)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
// @Failure 500 {object} string "error": "Internal Server Error"
// @Router /products [get]
func (clean *ProductHandler) GetAllProducts(c *gin.Context) {
	products, err := clean.ProductUsecase.GetAllProducts()
	if err != nil {
...
func (clean *ProductHandler) AllProducts(c *gin.Context) {
	products, err := clean.ProductUsecase.SeeAllProducts()
	if err != nil {
...
func (clean *ProductHandler) ShowSingleProduct(c *gin.Context) {
	id := c.Param("id")
	product, err := clean.ProductUsecase.GetSingleProduct(id)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (clean *ProductHandler) FilterProducts(c *gin.Context) {
	...
	Products, err := clean.ProductUsecase.FilterProducts(category, size, min, max)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (clean *ProductHandler) SearchProducts(c *gin.Context) {
	...
	products, err := clean.ProductUsecase.SearchProduct(Search.Search)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
