...
type OrderHandler struct {
	OrderUsecase interfaceUsecase.OrderUseCase
}

func NewOrderHandler(usecase interfaceUsecase.OrderUseCase) *OrderHandler{
...
// @Router /orders [post]
func (clean *OrderHandler) OrderFromCart(c *gin.Context) {
	...
	if OrderInput.PaymentID == 1 || OrderInput.PaymentID == 2 {
		OrderDetails, err := clean.OrderUsecase.ExecutePurchase(Token, OrderInput)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		} else {
		...
		OrderDetails, err := clean.OrderUsecase.ExecutePurchaseWallet(Token, OrderInput)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		} else {
...
// @Failure 500 {object} string "error": "Internal Server Error"
// @Router /checkout [get]
func (clean *OrderHandler) ViewCheckOut(c *gin.Context) {
	Token, err := c.Cookie("Authorisation")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	OrderDetails, err := clean.OrderUsecase.CheckOut(Token)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
// @Failure 500 {object} string "error": "Internal Server Error"
// @Router /orders [get]
func (clean *OrderHandler) ViewOrders(c *gin.Context) {
	Token, err := c.Cookie("Authorisation")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	OrderDetails, err := clean.OrderUsecase.ViewUserOrders(Token)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (clean *OrderHandler) CancelOrder(c *gin.Context) {
	Token, err := c.Cookie("Authorisation")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	err = clean.OrderUsecase.CancelOrder(Token, orderId, product_id)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (clean *OrderHandler) CancelOrderByAdmin(c *gin.Context) {
	...
	err = clean.OrderUsecase.CancelOrderByAdmin(cancel.UserID, cancel.OrderID, cancel.ProductID)
	if err != nil {
...
func (clean *OrderHandler) ShipOrderByAdmin(c *gin.Context) {
	...
	err = clean.OrderUsecase.ShipOrders(Ship.UserID, Ship.OrderID, Ship.ProductID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (clean *OrderHandler) DeliverOrderByAdmin(c *gin.Context) {
	...
	err = clean.OrderUsecase.DeliverOrder(Deliver.UserID, Deliver.OrderID, Deliver.ProductID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (clean *OrderHandler) ReturnOrder(c *gin.Context) {
	Token, err := c.Cookie("Authorisation")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	err = clean.OrderUsecase.ReturnOrder(Token, orderId, productId)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (clean *OrderHandler) SalesReportByDate(c *gin.Context) {
	...
	report, err := clean.OrderUsecase.ExecuteSalesReportByDate(startDate, endDate)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (clean *OrderHandler) SalesReportByPeriod(c *gin.Context) {
	period := c.PostForm("period")

	report, err := clean.OrderUsecase.ExecuteSalesReportByPeriod(period)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
	}
	c.Header("Content-Disposition", "attachment;filename=Invoice.pdf")
...
func (clean *OrderHandler) SalesReportByPayment(c *gin.Context) {
	startDateStr := c.PostForm("start")
	endDateStr := c.PostForm("end")
	...
	report, err := clean.OrderUsecase.ExecuteSalesReportByPaymentMethod(startDate, endDate, paymentmethod)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
	}
...
// @Failure 400 {object} string "error": "Bad Request"
// @Router /Invoice [get]
func (clean *OrderHandler) PrintInvoice(c *gin.Context) {
	Token, err := c.Cookie("Authorisation")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	pdf, err := clean.OrderUsecase.PrintInvoice(orderid, Token)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (clean *OrderHandler) ApplyCoupon(c *gin.Context) {
	Token, err := c.Cookie("Authorisation")
	if err != nil {
	...
	err = clean.OrderUsecase.ApplyCoupon(coupon, Token)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (clean *OrderHandler) SalesReportXL(c *gin.Context) {
	...
	report, err := clean.OrderUsecase.SalesReportXL(startDate, endDate)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
	}
